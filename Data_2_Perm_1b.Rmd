---
title: "Data_2_Perm_1b"
author: "Lauren Quesada"
date: "4/16/2022"
output: pdf_document
---

 X1 and X2 are interacting
```{r}

Data_Type_2 <- function(beta0, beta1, beta2, beta3, beta4, n_obs, rep){

 ###Define Variables 
 x1<- rnorm(n_obs) 
 x2<- rnorm(n_obs)

 y<- beta0 + beta1*x1 + beta2*x2 + beta3*x1*x2 + rnorm(n_obs)
...
output_5 <- pmap(list(b0, big_b1, big_b2, big_b3, big_b4, n_obs, rep), Data_Type_2)
```

```{r}
date()

results_5 <- map_dfr(output_5, Perm_Method_1b)

full_out_2_1b <- cbind(big_b, results_5)
